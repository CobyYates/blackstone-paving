...
> **`package-lock.json` is not committed** (it's git-ignored). Several Nuxt build
> deps ship platform-specific native binaries (oxc-parser, rollup, esbuild,
> lightningcss, sharp, …). A lockfile generated on macOS omits the Linux binaries
> (npm's optional-dependency bug, [npm/cli#4828](https://github.com/npm/cli/issues/4828)),
> so Cloudflare's Linux `npm ci` fails with *"Cannot find native binding"*. With no
> committed lockfile, Cloudflare runs `npm install`, which fetches the correct
> per-platform binaries. Node/npm are still pinned (`.nvmrc` + the `packageManager`
> field) so versions stay consistent — run `corepack enable` once if you want your
> local `npm` to honor the pin. Trade-off: builds aren't version-locked; if you
> need reproducible builds, generate the lockfile in Linux CI rather than on macOS.
